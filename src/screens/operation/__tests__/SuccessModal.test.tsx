...
describe('SuccessModal', () => {
  it('renders correctly', () => {
    const { getByText } = render(<SuccessModal isVisible onClose={jest.fn()} publicKey="publicKey" />);
    expect(getByText('Transaction successful!')).toBeTruthy();
  });

  it('calls onClose when the Close button is pressed', () => {
    const onClose = jest.fn();
    const { getByText } = render(<SuccessModal isVisible onClose={onClose} publicKey="publicKey" />);
